...
The body section is divided into four main sections: Top bar, About, Projects, and Contact each one is on different section tag. On the top of the web page there is an introduction about me include greeting message, my name, hook, and a quote.
...
Am API handelilng to an inspiration quote is also added to the JS file by craeting a function that fetch the API URL with a filtering on the quote tag and length for better visulization. An error message is shown to the user if connection error happend.
